...
- `baseURL`을 한 곳에서 관리 → 배포 시 쉽게 변경 가능한 역할
...
- `login`, `signup`, `logout` 함수 제공함
- 컴포넌트에서 URL을 직접 쓰지 않도록 관리(직접 쓰지 않아도 자동으로 특정 URL에 접근 가능하기 위함)
...
- 인증이 필요한 페이지는 `ProtectedRoute`로 감싸서 보호함
